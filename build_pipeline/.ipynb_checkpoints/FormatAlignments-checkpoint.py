...
def write_fasta( sequences ):
    ...
    print(f"SEQUENCES BEING WRITTEN OUT: {sequences}")
...
def format_j_genes(jalignments):
    ...
    if sys.platform == "darwin":
        ...
        print("DEBIAN DETECTED, MOVING THE STATIC FILE IN THE APPROPRIATE DIRECTORY WITHOUT MUSCLE")
        out_path = os.path.dirname(al_filename)
        pr = Popen( [ "cp", "/home/jupyter/ANARCI/all_js_aligned.fasta", out_path ], stdout=PIPE, stderr=PIPE)
    o, e = pr.communicate()
    ...
    print(f"new jalignments are here {new_jalignments}")
