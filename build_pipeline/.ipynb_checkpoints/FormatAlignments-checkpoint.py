...
def format_j_genes(jalignments):
    ...
    if sys.platform == "darwin":
        print("Running muscle for macOS")
        pr = Popen( [ "muscle_macOS", "-in", ffile, "-gapopen", "-10", "-out", al_filename, ], stdout=PIPE, stderr=PIPE )
    else:
        print("Running muscle for linux")
        print("Note muscle3 must be installed manually in $HOME to work as this is a patched install for Debian compatibility")
        print("ffile", ffile)
        print("al_filename", al_filename)
