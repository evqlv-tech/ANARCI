...
if ! which jackhmmer >/dev/null 2>&1; then
    echo "hmmer3 not found. attempting to install with pip"
    pip install hmmer
fi

echo "adding muscle3 to path. You must have muscle3 installed in location $HOME"
export PATH="$HOME/muscle3:$PATH"
